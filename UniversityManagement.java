...
import com.university.exception.profeesorNotFoundException;

public class UniversityManagement {
	private List<Student> students;// list to hold Students
	private List<Profeesor> professors;// list to hold professors.
	...
	public void addProfessor(String name) {
		professors.add(new Profeesor(name));
	}
	...
	public void assignCourseToProfessor(String professorName, String courseTitle)
			throws profeesorNotFoundException, CourseNotFoundException {
		Profeesor professor = findProfessorByName(professorName);
		Course course = findCourseByTitle(courseTitle);
		if (professor == null) {
			throw new profeesorNotFoundException("professor " + professorName + " not found  ");
		}
		if (course == null) {
			throw new CourseNotFoundException("Course :" + courseTitle + " not found");
		}
	...
	public void listProfessors() {
		if (professors.isEmpty()) {
			...
			for (Profeesor professor : professors) {
	...
	public void displayProfessorCourses(String professorName) throws profeesorNotFoundException {
		Profeesor professor = findProfessorByName(professorName);
		if (professor == null) {
	...
	public Profeesor findProfessorByName(String name) {
		for (Profeesor professor : professors) {
		...
		while (running) {
			...
			try {
				...
				case 5:
					...
					} catch (profeesorNotFoundException | CourseNotFoundException e) {
						System.out.println(e.getMessage()); // Handle exception
					}
				...
				case 10:
					...
					} catch (profeesorNotFoundException e) {
						System.out.println(e.getMessage()); // Handle exception
					}
					break;
				case 11:
